...
(function () {
	...
	function uploadFile (file) {		
		...
		*/
		...
		var url = base + 'admin/media/upload?folder='+path;			
		...
		xhr.onload = function() {
			if (this.responseText != '' && this.responseText.indexOf('</div>') == -1)
			{
				...
				img.setAttribute('alt', media.file_name);
				span.setAttribute('onclick', 'dagFiles.file.select(this)');
				span.setAttribute('title', media.file_name);
				span.setAttribute('data-filename', media.file_name);
			}           
        };
